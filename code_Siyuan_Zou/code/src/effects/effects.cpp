...
void effect_transition_compute(effect_transition_structure &effect_transition, character_structure &character)
{
	...
	// Fill the joint matrices with the interpolated values
	for (size_t k = 0; k < model.skeleton.joint_matrix_local.size(); ++k)
	{
		// Get the affine transformation of the source and destination animation
		affine_rt a_source = affine_rt::from_matrix(joint_source_anim_local[k]);
		affine_rt a_destination = affine_rt::from_matrix(joint_destination_anim_local[k]);

		// Interpolate the translation and rotation
		vec3 t = (1 - alpha_completion) * a_source.translation + alpha_completion * a_destination.translation;
		rotation_transform r = rotation_transform::lerp(a_source.rotation, a_destination.rotation, alpha_completion);
		quaternion q = r.get_quaternion();

		// Set the interpolated affine transformation
		model.skeleton.joint_matrix_local[k] = affine_rt(q, t).matrix();
	}
	model.skeleton.update_joint_matrix_local_to_global();
	// for (size_t k = 0; k < model.skeleton.joint_matrix_global.size(); ++k)
	// {
	// 	// Get the affine transformation of the source and destination animation
	// 	affine_rt a_source = affine_rt::from_matrix(joint_source_anim_global[k]);
	// 	affine_rt a_destination = affine_rt::from_matrix(joint_destination_anim_global[k]);
	...
	// 	// Set the interpolated affine transformation
	// 	model.skeleton.joint_matrix_global[k] = affine_rt(q, t).matrix();
	// }
...
void effect_walking_keyboard_event(effect_transition_structure &effect_transition, character_structure &character, cgp::input_devices const &inputs, effect_walking_structure const &effect_walking)
{
	...
	// If we just press the key down (or S), start the Walk cycle animation backward
	if (inputs.keyboard.last_action.is_pressed(GLFW_KEY_DOWN) || inputs.keyboard.last_action.is_pressed(GLFW_KEY_S))
	{
		effect_transition_start(effect_transition, character, effect_walking.walk_back_anim_name);
	}
	// If we just release the key down (or S), start the Idle cycle animation
	if (inputs.keyboard.last_action.is_released(GLFW_KEY_DOWN) || inputs.keyboard.last_action.is_released(GLFW_KEY_S))
	{
		effect_transition_start(effect_transition, character, effect_walking.idle_anim_name);
	}

	if (inputs.keyboard.last_action.is_pressed(GLFW_KEY_Q))
	{
		effect_transition_start(effect_transition, character, "Stab");
	}
	if (inputs.keyboard.last_action.is_released(GLFW_KEY_Q))
	{
		effect_transition_start(effect_transition, character, effect_walking.idle_anim_name);
	}
}
...
void effect_walking(effect_walking_structure &effect_walking, character_structure &character, cgp::input_devices const &inputs, effect_transition_structure const &effect_transition, perlin_noise_parameters terrain_parameters, float terrain_length)
{
	effect_transition_structure transition = effect_transition;
	animated_model_structure &model = character.animated_model;

	// affine_rt a_source = affine_rt::from_matrix(model.skeleton.joint_matrix_global[0]);
	// effect_walking.root_position = model.skeleton.joint_matrix_global[0].get_block_translation();
	...
	else if (inputs.keyboard.is_pressed(GLFW_KEY_RIGHT) || inputs.keyboard.is_pressed(GLFW_KEY_D))
	{
		// Turn right
		effect_walking.root_angle -= 3.0f * dt;
	}

	// rotation_transform r = rotation_axis_angle(vec3(0, 1, 0), effect_walking.root_angle - angle);
	// model.apply_transformation(vec3(0, 0, 0), r);

	if (inputs.keyboard.is_pressed(GLFW_KEY_UP) || inputs.keyboard.is_pressed(GLFW_KEY_W))
	{
		effect_walking.root_position += rotation_axis_angle({0, 1, 0}, effect_walking.root_angle) * vec3(0.0f, 0.0f, 1.0f) * 0.03;
		effect_walking.root_position.y = evaluate_terrain_height(effect_walking.root_position.x, -effect_walking.root_position.z, terrain_parameters, terrain_length) - evaluate_terrain_height(0, 0, terrain_parameters, terrain_length);
	}
	else if (inputs.keyboard.is_pressed(GLFW_KEY_DOWN) || inputs.keyboard.is_pressed(GLFW_KEY_S))
	{
		effect_walking.root_position -= rotation_axis_angle({0, 1, 0}, effect_walking.root_angle) * vec3(0.0f, 0.0f, 1.0f) * 0.03;
		// std::cout << effect_walking.root_position << std::endl;
		// std::cout << evaluate_terrain_height(effect_walking.root_position.x, -effect_walking.root_position.z, terrain_parameters, terrain_length) << std::endl;
		effect_walking.root_position.y = evaluate_terrain_height(effect_walking.root_position.x, -effect_walking.root_position.z, terrain_parameters, terrain_length) - evaluate_terrain_height(0, 0, terrain_parameters, terrain_length);
		// vec3 p = effect_walking.root_position;
		// p = rotation_transform::from_axis_angle({1, 0, 0}, Pi / 2) * p;
		// p.z = evaluate_terrain_height(p.x, p.y, terrain_parameters, terrain_length);
		// effect_walking.root_position = rotation_transform::from_axis_angle({1, 0, 0}, -Pi / 2) * p;
		// effect_walking.root_position.y = evaluate_terrain_height(effect_walking.root_position.x, -effect_walking.root_position.z, terrain_parameters, terrain_length);
	}
	...
	root.set_block_linear(R * root.get_block_linear());
